...
namespace brainerhub-demo-plugin\Cli;
...
class Setup {
	...
	public function __invoke( array $args, array $assoc_args ): void {
		...
		// Replace in files
		if (
			! $this->replaceInFiles( 'brainerhub-demo', $this->slug, [ '.*\.php', '.*\.js', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'brainerhub_demo', str_replace( '-', '_', $this->slug ), [ '.*\.php' ] )
			|| ! $this->replaceInFiles( 'brainerhub-demo-plugin', $this->namespace, [ '.*\.php', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'BRAINERHUB-DEMO-PLUGIN', strtoupper( $this->namespace ), [ '.*\.php' ] )
			|| ! $this->replaceInFiles( 'Brainerhub Demo Plugin', $this->name, [ '.*\.php', '.*README\.txt' ] )
		) {
	...
	private function rename_files(): void {
		if (
			! rename( $this->path . '/src/brainerhub-demo-plugin.php', $this->path . "/src/$this->namespace.php" )
			|| ! rename( $this->path . '/brainerhub-demo.php', $this->path . "/$this->slug.php" )
		) {
